...
import re
from models.athlete_comp_data import OutdoorScore, DoubleOutdoorScore


def generate_outdoor_scores(comp_year,url, comp_id):
...
def generate_double_720_scores(comp_year,url, comp_id):
    response = requests.get(url)
    if response.status_code == 200:
        soup = BeautifulSoup(response.content, "html.parser")
        scores = []
